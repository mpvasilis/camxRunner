...
public class CAMxSA extends CAMxSIMULATION {
    ...
    public CAMxSA(CAMxSIMULATION caMxSIMULATION, String CORES) {
    ...
    public void createJobFile(WRF wrf){
        ...
        try (PrintWriter out = new PrintWriter(jobfile_dir+"SA.camx.run")) {
            String JOBFILE = "#!/bin/csh\n" +
                    ...
                    "&SA_Control\n" +
                    "\n" +
                    " SA_File_Root      = '$OUTPUT/CAMx.$RUN."+wrf.getSIMULATION_START_DATE_format2().substring(0, wrf.getSIMULATION_START_DATE_format2().length() - 2)+"${CAL}',\n" +
                    " SA_Summary_Output = .true.,\n" +
                    "\n" +
                    " SA_Stratify_Boundary         = .false.,\n" +
                    " SA_Deposition_Output         = .false.,\n" +
                    " SA_Number_of_Source_Regions  = 1,\n" +
                    " SA_Number_of_Source_Groups   = 5,\n" +
                    " Use_Leftover_Group           = .false.,\n" +
                    " SA_Treat_SULFATE_Class       = .true.,\n" +
                    " SA_Treat_NITRATE_Class       = .true.,\n" +
                    " SA_Treat_SOA_Class           = .true.,\n" +
                    " SA_Treat_PRIMARY_Class       = .true.,\n" +
                    " SA_Treat_MERCURY_Class       = .true.,\n" +
                    " SA_Treat_OZONE_Class         = .true.,\n" +
                    " SA_Use_APCA                  = .false.,\n" +
                    "\n" +
                    " SA_Receptor_Definitions      = ' ',\n" +
                    " SA_Source_Area_Map(1)        = '$PTSRCE/source_area_map.txt ',\n" +
                    " SA_Use_Partial_SourceMap     = .false.,\n" +
                    " Partial_Source_Area_Map(1,1) = ' ',\n" +
                    " SA_PT_Override               = .false.,\n" +
                    "\n" +
                    " SA_Master_Restart            = '$OUTPUT/CAMx.$RUN."+wrf.getSIMULATION_START_DATE_format2().substring(0, wrf.getSIMULATION_START_DATE_format2().length() - 2)+"${YESTERDAY}.sa.inst',\n" +
                    "\n" +
                    " SA_Points_Group(1)           = '$SA/industry.bin',\n" +
                    " SA_Points_Group(2)           = '$SA/energy.bin',\n" +
                    " SA_Points_Group(3)           = '$SA/transport.bin',\n" +
                    " SA_Points_Group(4)           = '$SA/residential.bin',\n" +
                    " SA_Points_Group(5)           = '$SA/agriculture.bin',\n" +
                    "\n" +
                    " SA_Emiss_Group_Grid(1,1)     = ' ',\n" +
                    " SA_Emiss_Group_Grid(2,1)     = ' ',\n" +
                    " SA_Emiss_Group_Grid(3,1)     = ' ',\n" +
                    " SA_Emiss_Group_Grid(4,1)     = ' ',\n" +
                    " SA_Emiss_Group_Grid(5,1)     = ' ',\n" +
                    "\n" +
                    "/\n" +
                    "!----------------------------------------------------------------------" +
                    "ieof\n" +
